...
def create_user(user: User_create,session: Session):
    ...
    # with Session(engine) as session:(will be injected by fastapi/routes/users)
...
def update_user_with_id(user_id: int,userobj: User_create,session: Session):
    # with Session(engine) as session:(will be injected by fastapi/routes/users)
    #Business Logic
    if userobj.age < 18:
        raise HTTPException(
...
def patch_user_with_userid(userobj: UserUpdate,user_id: int,session: Session):
    # with Session(engine) as session:(will be injected by fastapi/routes/users)
    user=session.get(User,user_id)
    if not user:
        raise HTTPException(
